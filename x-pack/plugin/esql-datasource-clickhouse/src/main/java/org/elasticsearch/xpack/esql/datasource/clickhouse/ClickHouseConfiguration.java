...
public final class ClickHouseConfiguration extends DataSourceConfiguration {
    ...
    protected void validate(ValidationException errors) {
        ...
        // When a password is present, reject CRLF so it cannot be used for HTTP header injection:
        // the value is concatenated verbatim into X-ClickHouse-Key.
        String password = password();
        if (password != null && (password.indexOf('\r') >= 0 || password.indexOf('\n') >= 0)) {
            errors.addValidationError("password must not contain line-break characters");
        }
    }
    ...
    public static Configured<ClickHouseConfiguration> fromQueryConfig(Map<String, Object> raw) {
        return filterAndConstruct(raw, FIELDS, ClickHouseConfiguration::new);
    }
