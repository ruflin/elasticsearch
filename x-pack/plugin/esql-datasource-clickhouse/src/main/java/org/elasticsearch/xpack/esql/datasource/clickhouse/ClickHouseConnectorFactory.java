 ...
 */
class ClickHouseConnectorFactory implements ConnectorFactory, Closeable {
    ...
    @Override
    public String type() {
        return ClickHouseDataSourcePlugin.TYPE;
    }
    ...
    public void validateConfig(String location, Map<String, Object> config) {
        ...
        // Value-level checks (e.g. CRLF in password) for the inline EXTERNAL ... WITH path,
        // which does not go through the named data source's CRUD validator.
        ClickHouseConfiguration.fromQueryConfig(config);
    }
    ...
    public SourceMetadata resolveMetadata(String location, Map<String, Object> config) {
        ...
        return new SimpleSourceMetadata(schema, ClickHouseDataSourcePlugin.TYPE, location, null, null, null, resolvedConfig);
    }

    @Override
    public Connector open(Map<String, Object> config) {
        return new ClickHouseConnector(config, client());
    }
    ...
    /**
     * Shared JDK {@code HttpClient} for schema discovery and query execution. Created on first use
     * so factory unit tests that only parse URIs do not leak a selector thread. Closed by
     * {@link ClickHouseDataSourcePlugin#close()}.
     */
    private HttpClient client() {
        ...
        if (existing == null) {
            synchronized (this) {
                existing = httpClient;
                if (existing == null) {
                    existing = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(30)).build();
                    httpClient = existing;
                }
            }
        }
